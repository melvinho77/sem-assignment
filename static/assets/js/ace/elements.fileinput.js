...
(function($, undefined) {
    ...
    var Ace_File_Input = function(element, settings) {
        ...
        var parent_label = this.$element.closest('label').css({
            'display': 'block'
        })
    ...
    Ace_File_Input.prototype.show_file_list = function($files, inner_call) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (!this.well_style) this.$label.find('.ace-file-name').attr({
                'data-title': filename
            }).find(ace.vars['.icon']).attr('class', ace.vars['icon'] + fileIcon);
            else {
    ...
    var preview_image = function(file) {
        ...
        var imgLoaded = function(img) {
            ...
            if (thumb == null) {
                ...
                $(this).remove();
                deferred.reject({
                    code: Ace_File_Input.error['THUMBNAIL_FAILED']
                });
                return;
            }
        ...
        var imgFailed = function(img) {
            ...
            deferred.reject({
                code: Ace_File_Input.error['IMAGE_LOAD_FAILED']
            });
        }
        ...
        if (hasFile && file instanceof File) {
            ...
            reader.onerror = function(e) {
                deferred.reject({
                    code: Ace_File_Input.error['FILE_LOAD_FAILED']
                });
            }
